<?php
/*
------------------
Language: Português do Brasil (Brazilian Portuguese)
------------------
*/

$LANG['H_HOME'] = 'Início';

$LANG['H_SEARCH'] = 'Pesquisar';
$LANG['H_BROWSE'] = 'Explorar';
$LANG['H_COLLECTIONS'] = 'Coleções';
$LANG['H_SPECIES'] = 'Espécies';
$LANG['H_SPECIMENS'] = 'Espécimes';
$LANG['H_MAP'] = 'Mapa';
$LANG['H_TAXONOMIC_EXPLORER'] = 'Explorar Taxonomia';
$LANG['H_PEOPLE'] = 'Pessoas';
$LANG['H_CITE'] = 'Citação Recomendada';


$LANG['H_IMAGES'] = 'Imagens';
$LANG['H_IMAGE_BROWSER'] = 'Navegador de Imagens';
$LANG['H_IMAGE_SEARCH'] = 'Pesquisa de Imagens';

$LANG['H_INVENTORIES'] = 'Inventários Taxonômicos';
$LANG['H_CENTRAL_AMER'] = 'América Central';
$LANG['H_ECUADOR'] = 'Equador';
$LANG['H_SUBPOLAR'] = 'Região Subpolar do Sul';

$LANG['H_DYN_LISTS'] = 'Listas Taxonômicas Dinâmicas';

$LANG['H_MORE_INFO'] = 'Mais Informações';
$LANG['H_SYMBIOTA'] = 'Symbiota';
$LANG['H_HELP'] = 'Ajuda do Symbiota';
$LANG['H_SITEMAP'] = 'Mapa do Site';
$LANG['H_ABOUT_PROJECT'] = 'About Project';
$LANG['H_ABOUT'] = 'Cerca de';
$LANG['H_PARTNERS'] = '';
$LANG['H_CONTACTS'] = '';

$LANG['H_CONTACTS'] = 'Contato';
$LANG['H_PARTNERS'] = 'Parceiros';
$LANG['H_RESPONSIBLE'] = 'Responsáveis';

$LANG['H_WELCOME'] = 'Bem-vindo(a)';
$LANG['H_MY_PROFILE'] = 'Meu Perfil';
$LANG['H_MY_ACCOUNT'] = 'Meu Conta';
$LANG['H_PROFILE'] = 'Perfil';
$LANG['H_LOGOUT'] = 'Finalizar Sessão';
$LANG['H_LOGIN'] = 'Iniciar Sessão';
$LANG['H_NEW_ACCOUNT'] = 'Nova Conta';

$LANG['FOOTER'] = 'Os Bambus Nativos de Abya Yala são construídos sobre a plataforma <a href="https://symbiota.org/" target="_blank">Symbiota</a> personalizada com elementos temáticos de <a href="https://www.w3schools.com/w3css/default.asp" target="_blank">w3.css</a>; modificado e mantido por <a href="https://github.com/cdtyrrell" target="_blank">Christopher D. Tyrrell</a>';

?>